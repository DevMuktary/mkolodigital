...
// --- HANDLE UPDATES (STATUS, RESULT, REASON & FILE UPLOAD) ---
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['request_id'])) {
    $request_id = $_POST['request_id'];
    $new_status = $_POST['status'];
    $bvn_result = trim($_POST['bvn_result']);
    ...
    if (isset($_FILES['result_file']) && $_FILES['result_file']['error'] == 0) {
        ...
        $uploadDir = '../uploads/';
        $fileName = 'bvn-retrieval-' . uniqid() . '-' . basename($file['name']);
    ...
    if (empty($message)) {
        ...
        $bvn_to_save = !empty($bvn_result) ? $bvn_result : NULL;
        $file_to_save = !empty($result_file_path) ? $result_file_path : NULL;

        $sql = "UPDATE bvn_retrievals SET status = ?, bvn_result = ?, rejection_reason = ?, result_file_path = ? WHERE id = ?";
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("ssssi", $new_status, $bvn_to_save, $reason_to_save, $file_to_save, $request_id);
            if ($stmt->execute()) {
...
// Fetch all BVN retrieval requests
$requests = [];
$sql = "SELECT r.id, r.identifier_value, r.status, r.request_date, r.bvn_result, r.rejection_reason, r.result_file_path, u.full_name, u.email 
        FROM bvn_retrievals r JOIN users u ON r.user_id = u.id ORDER BY r.request_date DESC";
...
<head>
    ...
    <title>Manage BVN Retrievals</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .action-form select, .action-form input, .action-form textarea { width: 100%; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 8px; font-size: 14px; margin-bottom: 0.5rem; }
...
<body>
    <div class="container">
        <header class="top-header">
            <h3><i class="fas fa-search"></i> Manage BVN Retrievals</h3>
            <a href="dashboard.php"><i class="fas fa-arrow-left"></i> Back to Dashboard</a>
        </header>

        <?php if(!empty($message)): ?>
            <div class="message <?php echo $message_type === 'success' ? 'success' : 'error'; ?>"><?php echo $message; ?></div>
        <?php endif; ?>
                        ...
                        <tbody>
                            ...
                            <?php else: foreach ($requests as $req): ?>
                                <tr>
                                    ...
                                    <td>
                                        <strong>Date:</strong> <?php echo date("d M Y, g:ia", strtotime($req['request_date'])); ?><br>
                                        <strong>Submitted:</strong><pre><?php echo htmlspecialchars($req['identifier_value']); ?></pre>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <form class="action-form" method="POST" enctype="multipart/form-data">
                                            ...
                                            <label>BVN Result (Text):</label>
                                            <input type="text" name="bvn_result" placeholder="Enter 11-digit BVN (optional)" value="<?php echo htmlspecialchars($req['bvn_result'] ?? ''); ?>" maxlength="11">
                                            ...
                                            <input type="file" name="result_file" accept=".pdf,.zip,.jpg,.png">
                                            
                                            <?php if (!empty($req['result_file_path'])): ?>
                                                <div><a href="../<?php echo htmlspecialchars($req['result_file_path']); ?>" class="document-link" target="_blank">View Current File</a></div>
                                            <?php endif; ?>
