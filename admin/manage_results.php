...
// --- HANDLE UPDATES (STATUS, RESULT INFO & FILE UPLOAD) ---
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['request_id'])) {
    $request_id = $_POST['request_id'];
    $new_status = $_POST['status'];
    $result_info = trim($_POST['result_info']);
    ...
    if (isset($_FILES['result_file']) && $_FILES['result_file']['error'] == 0) {
        ...
        $uploadDir = '../uploads/';
        $fileName = 'result-' . uniqid() . '-' . basename($file['name']);
    ...
    if (empty($message)) { // Proceed only if there was no upload error
        $sql = "UPDATE result_requests SET status = ?, result_info = ?, result_file_path = ? WHERE id = ?";
        if ($stmt = $conn->prepare($sql)) {
            $info_to_save = !empty($result_info) ? $result_info : NULL;
            $file_to_save = !empty($result_file_path) ? $result_file_path : NULL;
            
            $stmt->bind_param("sssi", $new_status, $info_to_save, $file_to_save, $request_id);
            if ($stmt->execute()) {
...
// Fetch all result checker requests
$requests = [];
...
<head>
    ...
    <title>Manage Result Requests</title>
...
<body>
    <div class="container">
        <header class="top-header">
            <h3><i class="fas fa-graduation-cap"></i> Manage Result Requests</h3>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <input type="text" id="searchInput" class="search-box" placeholder="Search by name, email, or exam...">
            </div>
            <div class="card-body">
                        ...
                        <tbody>
                            ...
                            <?php else: foreach ($requests as $req): ?>
                                <tr>
                                    ...
                                    <td>
                                        <strong>Service:</strong> <?php echo htmlspecialchars($req['exam_type'] . ' ' . $req['service_type']); ?><br>
                                    ...
                                    <td>
                                        <form class="action-form" method="POST" enctype="multipart/form-data">
                                            ...
                                            <label>Result/PIN (Text):</label>
                                            <input type="text" name="result_info" placeholder="Enter PIN here if applicable" value="<?php echo htmlspecialchars($req['result_info'] ?? ''); ?>">
